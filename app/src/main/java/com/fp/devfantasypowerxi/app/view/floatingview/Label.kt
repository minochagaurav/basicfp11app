...
import com.fp.devfantasypowerxi.app.view.floatingview.Util.hasJellyBean
import com.fp.devfantasypowerxi.app.view.floatingview.Util.hasLollipop
...
class Label : AppCompatTextView {
    ...
    fun updateBackground() {
        ...
        if (mShowShadow) {
            layerDrawable = LayerDrawable(arrayOf(
                Shadow(),
                createFillDrawable()
            ))
    ...
    private fun createFillDrawable(): Drawable {
        ...
        drawable.addState(intArrayOf(R.attr.state_pressed), createRectDrawable(mColorPressed))
        drawable.addState(intArrayOf(), createRectDrawable(mColorNormal))
        if (hasLollipop()) {
            val ripple =
                RippleDrawable(ColorStateList(arrayOf(intArrayOf()), intArrayOf(mColorRipple)),
                    drawable,
                    null)
    ...
    fun onActionDown() {
        ...
        if (mBackgroundDrawable is StateListDrawable) {
            ...
            drawable.state = intArrayOf(R.attr.state_pressed)
        } else if (hasLollipop() && mBackgroundDrawable is RippleDrawable) {
            val ripple = mBackgroundDrawable as RippleDrawable
            ripple.state = intArrayOf(R.attr.state_enabled, R.attr.state_pressed)
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (mFab == null || mFab!!.onClickListener == null || !mFab!!.isEnabled) {
    ...
    private inner class Shadow  constructor() : Drawable() {
        ...
        private fun init() {
            ...
            if (!isInEditMode) {
                mPaint.setShadowLayer(mShadowRadius.toFloat(),
